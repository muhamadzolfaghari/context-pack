...
export function renderTargetSelector(state, version) {
  ...
  console.log("");
  console.log("  " + c.bold + c.cyan + "◆ CONTEXT LAB ENTERPRISE" + c.reset + " " + c.dim + ver + c.reset + " — " + c.bold + "Select Target LLM Profile" + c.reset);
  ...
  state.targetChoices.forEach(function (id, index) {
    ...
    if (id === null) {
      ...
      const line = "  " + pointer + " " + keyNum + " " + targetCol + budgetCol + ctxCol + descCol;
      if (active) {
...
export function renderBudgetSelector(state, version) {
  ...
  console.log("");
  console.log("  " + c.bold + c.cyan + "◆ CONTEXT LAB ENTERPRISE" + c.reset + " " + c.dim + ver + c.reset + " — " + c.bold + "Custom Token Budget Allocator" + c.reset);
  console.log("  " + c.dim + "Select token capacity ceiling or press Esc to return to profiles." + c.reset);
  console.log("  " + sep);
  ...
  BUDGET_LIST.forEach(function (budget, index) {
    ...
    const line = "  " + pointer + " " + keyNum + " " + col1 + col2 + col3;
    if (active) {
      console.log(c.inverse + stripAnsi(line) + c.reset);
    } else {
      console.log(line);
...
export function renderFocusModal(state, version) {
  ...
  console.log("");
  console.log("  " + c.bold + c.cyan + "◆ CONTEXT LAB ENTERPRISE" + c.reset + " " + c.dim + ver + c.reset + " — " + c.bold + "Task Objective & Dependency Focus" + c.reset);
  console.log("  " + c.dim + "Context Lab analyzes this task prompt to prioritize files, follow imports, and pull tests." + c.reset);
  console.log("  " + sep);
  console.log("  " + c.bold + "Objective:" + c.reset + " " + c.cyan + (state.focusInput || c.dim + "(type task, e.g. refactor auth token rotation and update tests)" + c.reset) + c.bold + "█" + c.reset);
  console.log("  " + sep);
...
export function renderRestoreModal(state, version) {
  ...
  console.log("");
  console.log("  " + c.bold + c.cyan + "◆ CONTEXT LAB ENTERPRISE" + c.reset + " " + c.dim + ver + c.reset + " — " + c.bold + "Apply AI Response & Pre-flight Inspection" + c.reset);
  ...
  if (state.applyPlan && state.applyPlan.length > 0) {
    ...
    console.log("  " + c.dim + "Impact Summary: " + c.reset + summaryBadge);
    console.log("  " + sep);
    ...
    if (state.applyPlan.length > 14) {
      console.log("    " + c.dim + "... and " + (state.applyPlan.length - 14) + " more files" + c.reset);
    }

    console.log("  " + sep);
    console.log("  " + c.dim + "🛡️ Enterprise Safety: Snapshots saved to .ctxlab/backups/ (revert anytime via 'ctxlab revert')" + c.reset);
    console.log("  " + sep);
    ...
    console.log("  " + sep);
    console.log("  " + c.dim + "No valid Markdown code blocks or JSON file modifications detected in clipboard." + c.reset);
    console.log("  " + sep);
    console.log("  " + c.bold + "1." + c.reset + " Prompt ChatGPT, Claude, or DeepSeek with your context pack.");
    console.log("  " + c.bold + "2." + c.reset + " Copy the chatbot's Markdown code blocks or JSON response (Cmd+C / Ctrl+C).");
    console.log("  " + c.bold + "3." + c.reset + " Press " + c.bold + "[r]" + c.reset + " here to inspect changes and apply them safely.");
    if (state.message) {
      console.log("  " + sep);
      console.log("  " + c.amber + state.message + c.reset);
    }
    console.log("  " + sep);
    console.log("  " + c.bold + "Esc" + c.reset + c.dim + " return to explorer" + c.reset + "\n");
  }
}
...
export function renderDoneModal(state, version) {
  ...
  console.log("");
  console.log("  " + c.bold + c.emerald + "✔ CONTEXT PACK BUILT & COPIED" + c.reset + " " + c.dim + ver + c.reset);
  console.log("  " + sep);

  if (state.builtPack) {
    const headroom = Math.max(0, state.builtPack.budget - state.builtPack.totalTokens);
    console.log("  " + padEnd(c.dim + "Files Selected:" + c.reset, 24) + c.bold + state.builtPack.selectedCount + c.reset + " of " + state.builtPack.candidateCount + " scanned");
    console.log("  " + padEnd(c.dim + "Token Payload:" + c.reset, 24) + c.bold + c.emerald + formatTokens(state.builtPack.totalTokens) + c.reset + " / " + formatTokens(state.builtPack.budget) + " safe budget limit");
    console.log("  " + padEnd(c.dim + "Response Headroom:" + c.reset, 24) + c.cyan + formatTokens(headroom) + " tokens free" + c.reset + c.dim + " (reserved for reasoning & output)" + c.reset);
    console.log("  " + padEnd(c.dim + "Target LLM Profile:" + c.reset, 24) + (state.builtPack.target ? c.cyan + state.builtPack.target.id + " (" + state.builtPack.target.modelFamily + ")" : "custom capacity") + c.reset);
    console.log("  " + padEnd(c.dim + "Export Format:" + c.reset, 24) + c.bold + state.format + c.reset);
    if (state.builtPack.focus) {
      console.log("  " + padEnd(c.dim + "Task Focus:" + c.reset, 24) + c.amber + state.builtPack.focus + c.reset);
    }
  }

  console.log("  " + sep);
  console.log("  " + (state.message || (c.emerald + "✔ Context pack copied to clipboard and ready for LLM." + c.reset)));
  console.log("  " + sep);
...
export function renderPreview(state, version) {
  ...
  console.log("");
  console.log("  " + c.bold + c.cyan + "◆ CONTEXT LAB" + c.reset + " " + c.dim + ver + c.reset + " " + c.dim + "│" + c.reset + " " + c.bold + (isFile ? "File Inspector: " : "Folder Contents: ") + c.reset + itemInfo);
  console.log("  " + sep);
...
export function renderBrowse(state, visible, version) {
  ...
  console.log("  " + brand + "  " + targetBadge + " " + formatBadge + " " + branchBadge + " " + gitBadge);
  ...
  console.log("  " + progressBar + "  " + c.dim + "│" + c.reset + "  " + estTokensText + "  " + c.dim + "│" + c.reset + "  " + headroomText + "  " + c.dim + "│" + c.reset + "  " + seedsCountText);
  console.log("  " + sep);
  ...
  if (state.focusPrompt) {
    console.log("  " + c.dim + "🎯 Task Focus: " + c.reset + c.amber + truncate(state.focusPrompt, cols - 24) + c.reset);
  }
  if (state.message) {
  ...
  if (currentItem && currentItem.type !== "parent") {
    ...
    console.log("  " + c.dim + "Space select · Tab search/tree · y copy pack · r apply AI response" + c.reset);
  }
